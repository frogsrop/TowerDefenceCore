...
public class DropdownMenu : MonoBehaviour
{
    ...
    public void OnButtonSpawn()
    {
        ...
        if (valueTowerMenu == 3) { _quantityTowers = 1000; }
        ...
        if (valueEnemiesMenu == 3) { _quantityEnemies = 1000; }

        valueSpeedShootMenu = _speedShootMenu.value;
        if (valueSpeedShootMenu == 0) { _speedShoot = 0f; }
        if (valueSpeedShootMenu == 1) { _speedShoot = 0.5f; }
        if (valueSpeedShootMenu == 2) { _speedShoot = 1f; }
        if (valueSpeedShootMenu == 3) { _speedShoot = 5f; }
